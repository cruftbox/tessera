...
void TAMC_GT911::reset() {
  ...
  if (hasInt) { pinMode(pinInt, OUTPUT); digitalWrite(pinInt, 0); }
  if (hasRst) { pinMode(pinRst, OUTPUT); digitalWrite(pinRst, 0); }
  delay(10);
  if (hasInt) digitalWrite(pinInt, addr==GT911_ADDR2);
  delay(1);
  if (hasRst) digitalWrite(pinRst, 1);
  delay(5);
  if (hasInt) digitalWrite(pinInt, 0);
  delay(50);
  if (hasInt) pinMode(pinInt, INPUT);
